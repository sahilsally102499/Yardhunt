    ...
    <div style={{ fontFamily: "'Georgia', serif", minHeight: "100vh", background: "#fdf6ec" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag-pill { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; letter-spacing: 0.5px; }
        ...
        input[type=text], input[type=date], input[type=time], select, textarea { width: 100%; padding: 10px 14px; border: 1.5px solid #d6c4a8; border-radius: 4px; background: #fffdf8; font-size: 15px; color: #3a2c1a; outline: none; transition: border-color 0.2s; }
      ...
      {/* Header */}
      <header style={{ background: "#1a0a05", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 26 }}>🍁</span>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 900, color: "#f5ddb4", letterSpacing: 1 }}>Yardhunt.ca</span>
        </div>
        <nav style={{ display: "flex", gap: 8 }}>
          <button onClick={() => { setView("browse"); setSelectedSale(null); }} style={{ background: view === "browse" ? "#c0392b" : "transparent", color: view === "browse" ? "white" : "#f5ddb4", border: "none", padding: "8px 18px", borderRadius: 4, cursor: "pointer", fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 14 }}>Browse Sales</button>
          <button onClick={() => setView("post")} style={{ background: view === "post" ? "#c0392b" : "transparent", color: view === "post" ? "white" : "#f5ddb4", border: "none", padding: "8px 18px", borderRadius: 4, cursor: "pointer", fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 14 }}>+ Post a Sale</button>
        </nav>
      </header>
      ...
      {view === "browse" && !selectedSale && (
        <div style={{ background: "linear-gradient(135deg, #1a0a05 0%, #6b1a1a 55%, #c0392b 100%)", padding: "52px 24px 40px", textAlign: "center" }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(30px, 6vw, 54px)", color: "#fff", fontWeight: 900, lineHeight: 1.1, marginBottom: 28 }}>
            Garage Sales &amp;<br />Yard Sales Across Canada
          </h1>
            ...
            <>
              ...
              <div style={{ background: "white", borderRadius: 8, padding: "32px 28px", boxShadow: "0 2px 16px rgba(0,0,0,0.08)", border: "1px solid #e8d9c4", display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. The Tremblay Family" value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  <div>
                    <label>Street Address *</label>
                    <input type="text" placeholder="123 Maple Street" value={form.address} onChange={e => setForm(f => ({...f, address: e.target.value}))} />
                  </div>
                  <div>
                    <label>City *</label>
                ...
                <div>
                  ...
                  <textarea rows={4} placeholder="What's for sale? Any special deals or highlights?" value={form.description} onChange={e => setForm(f => ({...f, description: e.target.value}))} style={{ resize: "vertical" }} />
                </div>
                <div>
                  <label>Categories (select all that apply)</label>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 6 }}>
                    {tagOptions.map(tag => (
                      <button key={tag} onClick={() => toggleTag(tag)} style={{ padding: "7px 14px", borderRadius: 20, border: `1.5px solid ${form.tags.includes(tag) ? (tagColors[tag] || "#c0392b") : "#d6c4a8"}`, background: form.tags.includes(tag) ? ((tagColors[tag] || "#c0392b") + "22") : "transparent", color: form.tags.includes(tag) ? (tagColors[tag] || "#c0392b") : "#7a5c3a", cursor: "pointer", fontSize: 13, fontWeight: 600, transition: "all 0.15s" }}>{tag}</button>
                    ))}
                  </div>
                </div>
